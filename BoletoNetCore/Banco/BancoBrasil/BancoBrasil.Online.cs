    ...
    partial class BancoBrasil : IBancoOnlineRest
    {
        ...
        public async Task<DownloadArquivoRetornoItem[]> DownloadArquivoMovimentacao(int numeroContrato, int codigoSolicitacao, int idArquivo, DateTime inicio, DateTime fim)
        {
            ...
            for (int i = 0; i < retorno.ListaRegistro.Count; i++)
            {
                ...
                var retornoItem = new DownloadArquivoRetornoItem
                {
                ...
                if (item.DataLiquidacaoBoleto != null && item.DataLiquidacaoBoleto != "")
                {
                    retornoItem.DataLiquidacao = this.ParseDateFromStringBB(item.DataLiquidacaoBoleto);
                }
                if (item.DataMovimentoRetorno != null && item.DataMovimentoRetorno != "")
                {
                    retornoItem.DataMovimentoLiquidacao = this.ParseDateFromStringBB(item.DataMovimentoRetorno);
                }
                if (item.DataCreditoPagamentoBoleto != null && item.DataCreditoPagamentoBoleto != "")
                {
                    retornoItem.DataPrevisaoCredito = this.ParseDateFromStringBB(item.DataCreditoPagamentoBoleto);
                }
                if (item.DataVencimentoBoleto != null && item.DataVencimentoBoleto != "")
                {
                    retornoItem.DataVencimentoTitulo = this.ParseDateFromStringBB(item.DataVencimentoBoleto);
                }
                items.Add(retornoItem);
            }
        ...
        private DateTime ParseDateFromStringBB(string dateString)
        {
            ...
            throw new FormatException($"Data inválida: {dateString}");
        }
